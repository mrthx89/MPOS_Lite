...
Namespace View
    Public Class frmDaftarBarang
        ...
        Private Sub cmdRefresh_Click(sender As Object, e As EventArgs) Handles cmdRefresh.Click
            Dim Daftar As New List(Of Model.Barang)
            Dim ObjFilter As New List(Of Model.Filter)
            Try
                If txtCari.Text <> "" Then
                    ObjFilter.Add(New Model.Filter With {.Spliter = Model.Filter.iSpliter.Atau, .Kondisi = Model.Filter.iKondisi.Baik, .Kolom = "mBarang.kode", .Value = "'%" & Repository.Utils.FixApostropi(txtCari.Text) & "%'"})
                    ObjFilter.Add(New Model.Filter With {.Spliter = Model.Filter.iSpliter.Atau, .Kondisi = Model.Filter.iKondisi.Baik, .Kolom = "mBarang.nama", .Value = "'%" & Repository.Utils.FixApostropi(txtCari.Text) & "%'"})
                    ObjFilter.Add(New Model.Filter With {.Spliter = Model.Filter.iSpliter.Atau, .Kondisi = Model.Filter.iKondisi.Baik, .Kolom = "mBarang.satuan", .Value = "'%" & Repository.Utils.FixApostropi(txtCari.Text) & "%'"})
                End If
                Daftar = rep.ApplikasiServices.GetDaftarBarang(ObjFilter)
        ...
        Private Sub cmdHapus_Click(sender As Object, e As EventArgs) Handles cmdHapus.Click
            Try
                If BS.Count >= 1 AndAlso rep.ApplikasiServices.HapusDataBarang(BS.Current) Then
                    cmdRefresh.PerformClick()
                End If
            Catch ex As Exception
                MetroMessageBox.Show(Repository.Public.FormMain, "Error : " & ex.Message, Repository.Public.NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Error)
