...
Namespace View
    Public Class frmDaftarKontak
        ...
        Private Sub cmdRefresh_Click(sender As Object, e As EventArgs) Handles cmdRefresh.Click
            Dim Daftar As New List(Of Model.Kontak)
            Dim ObjFilter As New List(Of Model.Filter)
            Try
                If txtCari.Text <> "" Then
                    ObjFilter.Add(New Model.Filter With {.Spliter = Model.Filter.iSpliter.Atau, .Kondisi = Model.Filter.iKondisi.Baik, .Kolom = "mkontak.nama", .Value = "'%" & Repository.Utils.FixApostropi(txtCari.Text) & "%'"})
                    ObjFilter.Add(New Model.Filter With {.Spliter = Model.Filter.iSpliter.Atau, .Kondisi = Model.Filter.iKondisi.Baik, .Kolom = "mkontak.alamat", .Value = "'%" & Repository.Utils.FixApostropi(txtCari.Text) & "%'"})
                End If
                Daftar = rep.ApplikasiServices.GetDaftarKontak(ObjFilter)
        ...
        Private Sub frmKontak_Load(sender As Object, e As EventArgs) Handles Me.Load
            cmdRefresh.PerformClick()
        End Sub

        Private Sub cmdBaru_Click(sender As Object, e As EventArgs) Handles cmdBaru.Click
            Dim x As New frmEntriKontak
            Dim Obj As New Model.Kontak
            Try
        ...
        Private Sub cmdHapus_Click(sender As Object, e As EventArgs) Handles cmdHapus.Click
            Try
                If BS.Count >= 1 AndAlso rep.ApplikasiServices.HapusDataKontak(BS.Current) Then
                    cmdRefresh.PerformClick()
                End If
            Catch ex As Exception
                MetroMessageBox.Show(Repository.Public.FormMain, "Error : " & ex.Message, Repository.Public.NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Error)
